...
- Input nominal uang: format ribuan otomatis, prefix `Rp`, angka lebih mudah dibaca, dan tombol nominal cepat sebagai penambah nilai (bukan replace)
...
### v1.0.68 — Sinkronisasi Gaji & Tenaga ↔ Piutang
**Isi rilis:**
- Menambahkan relasi opsional pekerja → pelanggan agar data Gaji & Tenaga bisa tersambung ke data Piutang tanpa input ulang.
- Tambah opsi `Sekalian bayar hutang` saat bayar upah individu maupun batch.
- Backup/restore diperbarui untuk membawa relasi pekerja/pembayaran potong hutang baru.
...
**Isi rilis:**
- Banner pengingat backup diperbaiki agar hanya tampil saat backup terakhir >= 7 hari.
- Dashboard menambahkan tombol aksi langsung pada empty state (`Catat Pembayaran`, `Lihat Hutang`).
- Kartu Tren Keuangan menampilkan rentang `Periode aktif` agar konteks data lebih jelas.
- Halaman Backup menambahkan tombol `Refresh Status` untuk memuat ulang status folder, backup terakhir, dan status Google Drive.

### v1.0.63 — Penyempurnaan Stabilitas Dashboard dan UX Navigasi
**Isi rilis:**
- Sinkronisasi status backup diperbaiki agar info di Dashboard/Sidebar lebih konsisten setelah backup manual.
- Optimasi fetch dashboard melalui pengaturan query agar lebih ringan dan stabil.
- Penyempurnaan UX navigasi sidebar (badge/status dan kejelasan menu aktif) dilanjutkan.
- Preset filter laporan dipoles agar pengalaman audit lebih jelas.

### v1.0.62 — Optimasi Navigasi Sidebar dan Kejelasan Status
**Isi rilis:**
- Struktur menu sidebar disesuaikan: grup `TRANSAKSI` diubah menjadi `PIUTANG` agar alur `Pelanggan`–`Hutang`–`Pembayaran` lebih konsisten.
- Sidebar menambahkan badge status cepat untuk `Backup & Restore` dan `Lisensi`.
- Item menu aktif diberi indikator visual tambahan agar lebih mudah dikenali.
- Header grup menu dan tampilan scrollbar sidebar dipoles agar navigasi lebih rapi.

### v1.0.61 — Stabilitas Startup dan Penyempurnaan UX Audit
**Isi rilis:**
- Perbaikan stabilitas loading awal aplikasi setelah update/install dengan fallback loading screen berbasis file lokal.
- Filter preset di tab Laporan kini punya indikator visual tombol aktif agar lebih jelas preset yang sedang dipakai.
- Tambah preset `Minggu ini` pada Laporan (Hutang & Keuangan).
- Tambah tombol `Buka Folder Backup` di halaman Backup (Electron).
- Dashboard menampilkan info ringkas `Backup manual terakhir`.

### v1.0.60 — Peningkatan UX Audit Laporan dan Backup
**Isi rilis:**
- Halaman Backup menampilkan status backup manual terakhir dalam format relatif + tanggal lengkap.
- Dashboard menambahkan shortcut cepat ke menu `Laporan` dan `Backup`.
- Fokus rilis: mempercepat alur audit tanpa menyentuh data inti pengguna.
...
**Isi rilis:**
- Workflow release dipisah menjadi 2 job: `Verify Quality Gate` dan `Build Installer and Upload`.
- Job release diatur bergantung pada verify (`needs`) agar packaging hanya berjalan jika validasi lolos.
- Penamaan job diperjelas agar monitoring dan troubleshooting di tab Actions lebih mudah.
