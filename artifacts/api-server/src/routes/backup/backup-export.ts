...
export async function handleExport(req: Request, res: Response): Promise<void> {
  ...
  // ── Semua query dibungkus dalam satu transaction untuk konsistensi snapshot ──
  // better-sqlite3 bersifat SINKRON — sqliteRaw.transaction() menjamin semua
  // SELECT di bawah membaca state database yang sama (tidak tercampur dengan
  // INSERT/UPDATE/DELETE yang terjadi bersamaan dari request lain).
  // Drizzle `.all()` adalah operasi sinkron di better-sqlite3.
  let payload: Record<string, unknown>;

  try {
    const buildPayload = sqliteRaw.transaction(() => {
      const usahaList = db.select().from(usahaTable).where(eq(usahaTable.id, usahaId)).all();
      const usaha = usahaList[0];
      if (!usaha) return null;

      const pelangganList = db.select().from(pelangganTable).where(eq(pelangganTable.usahaId, usahaId)).all();
      const hutangList = db.select().from(hutangTable).where(eq(hutangTable.usahaId, usahaId)).all();
      const pembayaranList = db.select().from(pembayaranTable).where(eq(pembayaranTable.usahaId, usahaId)).all();
      const keuanganList = db.select().from(keuanganTable).where(eq(keuanganTable.usahaId, usahaId)).all();
      const barangList = db.select().from(barangTable).where(eq(barangTable.usahaId, usahaId)).all();
      const transaksiStokList = db.select().from(transaksiStokTable).where(eq(transaksiStokTable.usahaId, usahaId)).all();
      const transaksiKasirList = db.select().from(transaksiKasirTable).where(eq(transaksiKasirTable.usahaId, usahaId)).all();
      const pekerjaList = db.select().from(pekerjaTable).where(eq(pekerjaTable.usahaId, usahaId)).all();
      const upahList = db.select().from(upahPekerjaTable).where(eq(upahPekerjaTable.usahaId, usahaId)).all();
      const bayarUpahList = db.select().from(bayarUpahTable).where(eq(bayarUpahTable.usahaId, usahaId)).all();
      const pengaturanList = db.select().from(pengaturanTable).where(eq(pengaturanTable.usahaId, usahaId)).all();
      const suppliersList = db.select().from(suppliersTable).where(eq(suppliersTable.usahaId, usahaId)).all();

      // Ambil semua item kasir sekaligus (tetap dalam transaction yang sama)
      const transaksiKasirItemList: Array<typeof transaksiKasirItemTable.$inferSelect> = [];
      for (const k of transaksiKasirList) {
        const items = db.select().from(transaksiKasirItemTable).where(eq(transaksiKasirItemTable.transaksiKasirId, k.id)).all();
        transaksiKasirItemList.push(...items);
      }
    ...
    res.status(500).json({ error: "Gagal mengekspor data backup.", detail: message });
    return;
  }
