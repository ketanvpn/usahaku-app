...
export default function PembayaranPage() {
  ...
  const { data: pembayaranList, isLoading } = useGetPembayaranList({
    pelanggan_id: filterPelanggan
  });
  
  const { data: pelangganList } = useGetPelangganList();
  
  // Only fetch active debts for the selected customer in the form
  const { data: hutangAktifList } = useGetHutangList({
    pelanggan_id: formPelangganId || undefined,
    status: "aktif"
  }, {
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 pt-2">
            <div className="space-y-2 border-b pb-4 border-border">
              ...
              <Select onValueChange={(v) => { setFormPelangganId(parseInt(v)); form.setValue("hutang_id", 0); }} value={formPelangganId?.toString() || ""}>
                <SelectTrigger>
                  ...
                  <FormField control={form.control} name="hutang_id" render={({ field }) => (
                    <FormItem>
                      ...
                      <Select onValueChange={(v) => {
                        ...
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue placeholder="Pilih nota hutang..." />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                    ...
                    <>
                      ...
                      <FormField control={form.control} name="nominal_bayar" render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl><Input type="number" min="1" max={selectedHutang.sisa_hutang} {...field} /></FormControl>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Nota Hutang (ID)</TableHead>
                ...
                {!pembayaranList || pembayaranList.length === 0 ? (
                  ...
                  pembayaranList.map((p) => (
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <Link href={`/hutang/${p.hutang_id}`} className="hover:underline text-muted-foreground text-sm">
                          #{p.hutang_id}
                        </Link>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" onClick={() => { setSelectedPembayaran(p); setIsDeleteDialogOpen(true); }} title="Hapus/Batal" className="text-destructive">
